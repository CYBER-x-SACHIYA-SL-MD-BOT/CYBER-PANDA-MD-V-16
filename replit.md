...
| File/Dir         | Purpose                                         |
|------------------|-------------------------------------------------|
| `setup.js`       | Main entry — connects to WhatsApp, runs Express |
...
- `SESSION_ID` — WhatsApp session (required). Format: `Panda-<base64-gzip>`
...
- **Security**: `package.json` author field must be `"setup"` — integrity check in `setup.js` validates this on startup
