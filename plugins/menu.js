...
const REPO    = 'https://github.com/CYBER-x-SACHIYA-SL-MD-BOT/CYBER-PANDA-MD-V-16';
...
async function sendCallLogMenu(sock, jid, menuText, imgUrl) {
    ...
    const quotedCtx = {
        ...
        externalAdReply: {
            title:                 `${getStr('botName') || '🐼 ᴄʏʙᴇʀ ᴘᴀɴᴅᴀ ᴍᴅ 🐼'} — Command Menu`,
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        const botName  = getStr('botName') || config.BOT_NAME || '🐼 ᴄʏʙᴇʀ ᴘᴀɴᴅᴀ ᴍᴅ 🐼';
        const ownerNum = `+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}`;
        const mode     = (config.MODE || 'public').toUpperCase();
        const pfx      = prefix || '.';
        const imgUrl   = getStr('pic1') || config.ALIVE_IMG || 'https://i.ibb.co/4ZLVMbZN/IMG-20250726-WA0090.jpg';
